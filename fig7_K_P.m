...
Karr=[2 3 4 5 6 7 8 9 10];
snrdB=[-5 -2 1 4 7 10 13 17 20];
gnrt = zeros(length(Karr), length(snrdB)); % 存储g的值
grt = zeros(length(Karr), length(snrdB)); % 存储g的值
for cir=1:9   
    num_itr = 100;%总时隙数
    num1=1;
    num2=1;
...
[K, P] = meshgrid(Karr, snrdB);
figure(1);

% 绘制 g1 曲面图
surf1 = surf(P, K, gnrt', 'EdgeColor', 'black'); % 绘制 g1 曲面，显示网格线
surf1.FaceColor = 'interp'; % 启用插值颜色
surf1.FaceAlpha = 0.8; % 设置透明度

% 设置坐标轴标签和标题
xlabel('$P$(dB)', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Arial', 'Rotation', 25);
ylabel('$K$', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Arial', 'Rotation', -25);
zlabel('AoI', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Arial'); % Z轴标签
% title('NOMA-NRT'); % 标题

% 显示完整的刻度值
xticks(snrdB); % 设置 x 轴刻度为 snrdB 的所有值
yticks(Karr);  % 设置 y 轴刻度为 Karr 的所有值
...
% 绘制 g2 曲面图
surf2 = surf(P, K, grt', 'EdgeColor', 'black'); % 绘制 g2 曲面，显示网格线
...
% surf1.FaceColor = 'interp'; % 启用插值颜色
% surf1.FaceAlpha = 0.8; % 设置透明度
% hold on; % 保留当前图，继续绘制下一个曲面
% 
% % 绘制 g2 曲面图
% surf2 = surf(P, K, grt', 'EdgeColor', 'black'); % 绘制 g2 曲面，显示网格线
% surf2.FaceColor = 'interp'; % 启用插值颜色
% surf2.FaceAlpha = 0.5; % 设置透明度（更低透明度区分两个曲面）
% 
% % 设置坐标轴标签和标题
% xlabel('$P$(dB)', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Arial', 'Rotation', 25);
% ylabel('$K$', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Arial', 'Rotation', -25);
% zlabel('AoI', 'Interpreter', 'latex', 'FontSize', 14, 'FontName', 'Arial'); % Z轴标签
% title('NOMA-NRT and NOMA-RT'); % 标题
% 
% % 显示完整的刻度值
% xticks(snrdB); % 设置 x 轴刻度为 snrdB 的所有值
% yticks(Karr);  % 设置 y 轴刻度为 Karr 的所有值
...
% 
% % 添加图例
% legend({'NOMA-NRT', 'NOMA-RT'}, 'Location', 'northeast'); % 添加图例
% hold off; % 释放当前图的占用
